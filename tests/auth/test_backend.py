...
async def test_create_refresh_token(local_cache):
    manager = RefreshTokenManager(local_cache)
...
async def test_verify_refresh_token(local_cache):
    manager = RefreshTokenManager(local_cache)
    token = await manager.create_refresh_token(1, "Mozilla/5.0")
...
async def test_verify_invalid_token(local_cache):
    manager = RefreshTokenManager(local_cache)
...
async def test_revoke_token(local_cache):
    manager = RefreshTokenManager(local_cache)
    token = await manager.create_refresh_token(1, "Mozilla/5.0")
...
async def test_revoke_all_user_tokens(local_cache):
    manager = RefreshTokenManager(local_cache)
...
async def test_revoke_all_user_tokens_preserves_new_tokens(local_cache):
    manager = RefreshTokenManager(local_cache)
...
async def test_token_with_no_device_info(local_cache):
    manager = RefreshTokenManager(local_cache)
